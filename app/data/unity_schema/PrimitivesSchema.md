...
| `direction_mode` | `string` | Determines how the knockback direction vector is calculated. Valid options: 'HitNormal' (perpendicular to the collision surface), 'SourceForward' (opposite to the caster's facing direction, ideal for recoil), 'FromHitPoint' (from the exact contact point on the collider boundary to the target's center — unreliable for AoE), 'FromColliderCenter' (from the center of the triggering collider — i.e. the explosion origin — to the target's center; **use this for all explosive/AoE projectiles** so every target is pushed radially outward from the blast), or 'SourceToTarget' (from the caster's position to the target's position). |
